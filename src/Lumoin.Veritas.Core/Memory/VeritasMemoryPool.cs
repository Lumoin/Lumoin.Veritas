...
/// <summary>
/// A thread-safe memory pool that returns memory of exactly the requested size.
/// The pool automatically creates size-specific internal sub-pools to optimize
/// allocation patterns for different buffer sizes.
/// </summary>
/// <typeparam name="T">The type of memory elements to be reserved, typically <see cref="byte"/>.</typeparam>
/// <remarks>
/// <para>
/// This pool guarantees exact-size allocations. Unlike <see cref="ArrayPool{T}.Shared"/>
/// which may return larger buffers, every <see cref="Rent"/> call returns memory of
/// precisely the requested size. This is critical for RDF term interning where
/// <see cref="Utf8StringPool"/> slices interned values from pool-managed buffers.
/// </para>
/// <para>
...
/// for smaller buffers and fewer for larger buffers.
/// </para>
/// </remarks>
[DebuggerDisplay("VeritasMemoryPool<{typeof(T).Name,nq}>: Slabs={totalSlabs}, Active={activeRentals}, Allocated={totalMemoryAllocated} bytes")]
public class VeritasMemoryPool<T>: MemoryPool<T>
{
    /// <summary>
    ...
    /// </summary>
    private static ActivitySource ActivitySource { get; } = new("VeritasMemoryPool");
    ...
    /// Disable for high-frequency workloads where tracing overhead is unacceptable.
    /// </summary>
    public bool TracingEnabled { get; }

    /// <summary>
    /// The process-unique identity of this pool instance. Every observable measurement the
    /// instance reports carries it under <see cref="VeritasMetrics.MemoryPoolInstanceTag"/>,
    /// so a metrics consumer can attribute a measurement to this instance among the many
    /// pools (of any element type) sharing the instrument names.
    /// </summary>
    public int InstanceId { get; }

    /// <summary>
    /// The instance tag every observable measurement carries — cached so observation allocates
    /// nothing (the identity boxes exactly once, at construction).
    /// </summary>
    private KeyValuePair<string, object?> InstanceTag { get; }
    ...
    /// and fewer for larger ones, tuned for common RDF term sizes.
    /// </summary>
    ...
    /// </summary>
    private static Lazy<VeritasMemoryPool<T>> SharedInstance { get; } =
        new(static () => new VeritasMemoryPool<T>());

    /// <summary>
    /// Gets a shared singleton instance of the memory pool.
    /// </summary>
    /// <value>A singleton instance of the memory pool.</value>
    /// <remarks>
    ...
    /// </remarks>
    public static new VeritasMemoryPool<T> Shared => SharedInstance.Value;

    /// <summary>
    /// Initializes a new instance with default settings.
    /// </summary>
    public VeritasMemoryPool()
        : this(new Meter(VeritasMetrics.MeterName, "1.0.0"))
    {
    }
        ...
        bool tracingEnabled = true)
    {
        ...
        IsDisposed = false;
        InstanceId = VeritasMemoryPoolIdentity.Next();
        InstanceTag = new KeyValuePair<string, object?>(VeritasMetrics.MemoryPoolInstanceTag, InstanceId);
        ...
        meter.CreateObservableUpDownCounter(
            ...
            ObserveAllocationEfficiency,
            "percent",
            "Percentage of allocated memory currently in use.");

        BufferSizeHistogram = meter.CreateHistogram<int>(
            VeritasMetrics.MemoryPoolBufferSizeDistribution,
            "bytes",
            "Distribution of requested buffer sizes.");

        RentSuccessCounter = meter.CreateCounter<long>(
            VeritasMetrics.MemoryPoolRentOperationsTotal,
            "operations",
            "Total number of successful rent operations.");

        ReturnCounter = meter.CreateCounter<long>(
            VeritasMetrics.MemoryPoolReturnOperationsTotal,
            "operations",
            "Total number of memory return operations.");
    }

    /// <summary>Observes the total number of slabs created across all buffer sizes.</summary>
    /// <returns>The total slab count, tagged with this instance's identity.</returns>
    private Measurement<int> ObserveTotalSlabs()
    {
        return new Measurement<int>(totalSlabs, InstanceTag);
    }

    /// <summary>Observes the total memory allocated across all slabs.</summary>
    /// <returns>The total allocated bytes, tagged with this instance's identity.</returns>
    private Measurement<long> ObserveTotalMemoryAllocated()
    {
        return new Measurement<long>(totalMemoryAllocated, InstanceTag);
    }

    /// <summary>Observes the number of currently rented memory segments.</summary>
    /// <returns>The active rental count, tagged with this instance's identity.</returns>
    private Measurement<int> ObserveActiveRentals()
    {
        return new Measurement<int>(activeRentals, InstanceTag);
    }
    ...
    public override IMemoryOwner<T> Rent(int bufferSize)
    {
        ...
        using(LockObject.EnterScope())
        {
            ...
            foreach(Slab<T> slab in slabList)
            {
    ...
    public int TrimExcess()
    {
        ...
        using(LockObject.EnterScope())
        {
            foreach(List<Slab<T>> slabList in Slabs.Values)
            {
                for(int i = slabList.Count - 1; i >= 0; i--)
                {
                    Slab<T> slab = slabList[i];
                    if(slab.IsFull)
                    {
    ...
    /// <summary>
    /// Observes the current allocation efficiency — the percentage of allocated segments
    /// currently rented — tagged with this instance's identity.
    /// </summary>
    /// <returns>The efficiency percentage measurement; zero when no segments exist.</returns>
    private Measurement<double> ObserveAllocationEfficiency()
    {
        int currentTotalSegments = totalSegments;
        int currentActiveRentals = activeRentals;
        double efficiency = currentTotalSegments == 0
            ? 0.0
            : (double)currentActiveRentals / currentTotalSegments * 100.0;

        return new Measurement<double>(efficiency, InstanceTag);
    }
    ...
    internal class Slab<TElement>: IDisposable
    {
        ...
        /// Thrown when <paramref name="segmentSize"/> or <paramref name="segmentCount"/>
        ...
        /// <summary>
        /// Releases all resources used by this slab.
        /// </summary>
        public void Dispose()
        {
            if(!IsDisposed)
    ...
    /// <summary>
    /// Provides ownership of a memory segment rented from a <see cref="VeritasMemoryPool{T}"/>.
    /// Automatically returns the memory to the pool when disposed.
    /// </summary>
    ...
    private class ExactSizeMemoryOwner<TOwner>: IMemoryOwner<TOwner>
    {
        ...
        /// <summary>
        /// The slab that provided the segment.
        /// </summary>
        private VeritasMemoryPool<TOwner>.Slab<TOwner> Slab { get; }

        /// <summary>
        /// The memory pool that owns the slab.
        /// </summary>
        private VeritasMemoryPool<TOwner> Pool { get; }

        /// <summary>
        /// Indicates whether this owner has been disposed.
        /// </summary>
        ...
        public ExactSizeMemoryOwner(
            ...
            VeritasMemoryPool<TOwner>.Slab<TOwner> slab,
            VeritasMemoryPool<TOwner> pool,
        ...
        /// <summary>
        /// Returns the managed memory to the pool. The lifecycle activity is finalized
        /// with a return event and then disposed.
        /// </summary>
        /// <remarks>
        ...
        public void Dispose()
        {
            if(!Disposed)
            {
                ...
                catch(ObjectDisposedException ex)
                {
                    ...
                    //Expected during shutdown or when the pool is disposed
