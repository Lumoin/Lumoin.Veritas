...
/// <summary>
/// An RDF 1.2 annotation (a reifier <c>~</c> or an annotation block <c>{| … |}</c>) the parser could
/// not parse. Recovery emits this in place of a well-formed <see cref="Annotation"/>, spanning the
/// tokens it skipped to resynchronise.
/// </summary>
/// <remarks>
...
/// <see cref="SkippedTokens"/> records the consumed tokens. An error annotation contributes nothing to
/// the document's emitted quads.
/// </remarks>
[DebuggerDisplay("ErrorAnnotation {ExpectedProductionText,nq} ({SkippedTokens.Length} skipped) #{NodeId}")]
public sealed class ErrorAnnotation: Annotation
{
    /// <summary>
    /// Initialises a new <see cref="ErrorAnnotation"/>.
    /// </summary>
