...
/// <summary>
/// The entropy-coded gap codec's contract: every monotone distribution —
/// gappy at several universe-to-count ratios, dense, duplicate-laden,
/// clustered runs, geometric-skewed, single, empty, full-range —
/// round-trips losslessly through build and decode, at every window
/// offset including those straddling block and lane boundaries, and
/// descending input is rejected. A plain array is the identity oracle.
/// </summary>
[TestClass]
internal sealed class AnsGapSequenceTests
{
    ...
    private static uint[] DecodeAll(AnsGapSequence sequence)
    {
    ...
    {
        AnsGapSequence sequence = AnsGapSequence.Build(values);

        Assert.AreEqual(values.Length, sequence.Count);
        Assert.AreSequenceEqual(values, DecodeAll(sequence));

        //Windows at offsets that straddle block and lane boundaries: a
        //single element, a sub-block run, a cross-block run, and the tail.
    ...
    /// <summary>Enumerates window offsets and lengths exercising leading, interior, boundary-straddling, lane-offset, and trailing slices.</summary>
    ...
    private static IEnumerable<(int Start, int Count)> WindowsOver(int length)
    {
        ...
        //The odd starts (1, 2, 3) deliberately land off lane-zero so a
        //window can begin mid-interleave-cycle.
        int[] starts = [0, 1, 2, 3, AnsGapSequence.BlockLength - 1, AnsGapSequence.BlockLength, AnsGapSequence.BlockLength + 1, AnsGapSequence.BlockLength + 2, (2 * AnsGapSequence.BlockLength) + 3, length - 1];
        int[] counts = [1, 2, 3, AnsGapSequence.BlockLength, AnsGapSequence.BlockLength + 5, length];
    ...
    {
        AnsGapSequence sequence = AnsGapSequence.Build([]);
    ...
    {
        //Long constant runs: every gap is zero, so the length model is
        //single-symbol {0} and the nibble stream is empty — both
        //degenerate cases.
        ...
        //with steps, so zero gaps and non-zero gaps share the stream.
    ...
    {
        //Universe equals count: each gap is one, a single-symbol length
        //model and a single-symbol nibble model {1}.
        ...
        //Dense with a constant base offset: the first gap is large, the
        //rest are one, exercising a mixed nibble model.
    ...
    [TestMethod]
    public void GappySequenceRoundTripsAcrossRatios()
    {
        //Several universe-to-count ratios: each widens the gap model.
        foreach(ulong gapRange in (ulong[])[8, 64, 512, 8_000, 400_000])
        {
            ...
            uint[] values = new uint[ValueCount];
            ulong current = 0;
            ulong state = MixerSeed + gapRange;
            for(int i = 0; i < values.Length; i++)
            {
                state = Mix(state);
    ...
    [TestMethod]
    public void GeometricSkewedGapsRoundTrip()
    {
        //Geometric-skewed gaps: many tiny gaps and a long tail of large
        //ones, the shape entropy coding rewards most.
        const int ValueCount = 8_000;
        const ulong MixerSeed = 71;
        ...
        for(int i = 0; i < values.Length; i++)
        {
            ...
            //A geometric gap: the number of low bits the mixer agrees on
            //sets the magnitude, so small gaps dominate.
            int magnitude = System.Numerics.BitOperations.TrailingZeroCount(state | (1UL << 40));
            ulong gap = 1UL << magnitude;
            current += gap;
            values[i] = (uint)Math.Min(current, uint.MaxValue);
        }
    ...
    {
        //Runs of consecutive values separated by large jumps — gaps of
        //one punctuated by wide gaps, a bimodal model.
    ...
    public void FullRangeExtremesRoundTrip()
    {
        ...
        //A long monotone walk spanning the whole universe: every gap is
        //near the full-byte magnitude.
    ...
    {
        foreach(int length in (int[])[1, AnsGapSequence.BlockLength - 1, AnsGapSequence.BlockLength, AnsGapSequence.BlockLength + 1, (2 * AnsGapSequence.BlockLength) + 1, (5 * AnsGapSequence.BlockLength) + 13])
        {
    ...
    {
        const int ValueCount = (7 * AnsGapSequence.BlockLength) + 19;
        ...
        //Every single-element window across the sequence, then wide
        //windows starting at non-block-aligned and off-lane offsets.
        ...
        foreach((int start, int count) in (ValueTuple<int, int>[])[(3, 200), (AnsGapSequence.BlockLength - 5, 300), (AnsGapSequence.BlockLength + 7, AnsGapSequence.BlockLength), (ValueCount - 50, 50)])
        {
            uint[] window = new uint[count];
            sequence.Decode(start, count, window);
            for(int i = 0; i < count; i++)
    ...
    [TestMethod]
    public void ManySmallRandomLengthSequencesRoundTrip()
    {
        //Renormalisation edges hide in odd lengths and small alphabets;
        //sweep many short deterministic sequences against the identity
        //oracle, varying both length and gap magnitude.
        const int SequenceCount = 600;
        ulong state = 0x5151515151515151UL;

        for(int s = 0; s < SequenceCount; s++)
        {
            state = Mix(state);
            int length = (int)(state % 70);
            state = Mix(state);
            ulong gapRange = 1 + (state % 2_000);

            uint[] values = new uint[length];
            ulong current = state % 17;
            for(int i = 0; i < length; i++)
            {
                state = Mix(state);
                current += state % gapRange;
            ...
            AnsGapSequence sequence = AnsGapSequence.Build(values);
            Assert.AreSequenceEqual(values, DecodeAll(sequence), $"sequence {s} of length {length}");

            //Decode every window of this short sequence against the oracle.
            for(int start = 0; start < length; start++)
            {
                for(int count = 1; start + count <= length; count++)
                {
                    uint[] window = new uint[count];
                    sequence.Decode(start, count, window);
                    for(int i = 0; i < count; i++)
                    {
                        Assert.AreEqual(values[start + i], window[i], $"sequence {s} window ({start},{count}) at {i}");
                    }
                }
            }
        }
    ...
    {
        Assert.ThrowsExactly<ArgumentException>(() => AnsGapSequence.Build([5U, 4U]));
        Assert.ThrowsExactly<ArgumentException>(() => AnsGapSequence.Build([1U, 2U, 3U, 2U]));
        Assert.ThrowsExactly<ArgumentException>(() => AnsGapSequence.Build([uint.MaxValue, 0U]));
    }

    [TestMethod]
    public void DecodeRejectsOutOfRangeWindows()
    {
        AnsGapSequence sequence = AnsGapSequence.Build([1U, 2U, 3U, 4U]);
    ...
    {
        //A clustered sequence with many one-gaps compresses well below a
        //raw uint array yet reports a positive, model-and-directory
        //inclusive bit count.
        ...
        AnsGapSequence denseSequence = AnsGapSequence.Build(dense);
