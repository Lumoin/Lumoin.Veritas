...
/// <summary>
/// Integrity gate for the CITE GML corpus: every file listed in <c>MANIFEST.sha256</c> is present
...
/// <c>Geo/CiteGmlCorpus/PROVENANCE.md</c>, never regenerated here — and the clearance
/// outcomes in <see cref="CiteGmlCorpusExpectations"/> rest on these exact bytes.
/// </summary>
[TestClass]
internal sealed class CiteGmlCorpusManifestTests
{
    /// <summary>The number of artifacts the manifest pins; the vendored universe never drifts silently.</summary>
    private const int PinnedArtifactCount = 87;
    ...
    public async Task EveryManifestEntryExistsWithMatchingSha256()
    {
        ...
        {
            string path = CiteGmlCorpusPaths.GetPath(relativePath);
    ...
    public async Task NoCorpusFileExistsOutsideTheManifest()
    {
        ...
        foreach(string path in Directory.EnumerateFiles(CiteGmlCorpusPaths.CorpusRoot, "*.xml", SearchOption.AllDirectories))
        {
            string relativePath = Path.GetRelativePath(CiteGmlCorpusPaths.CorpusRoot, path).Replace(Path.DirectorySeparatorChar, '/');
    ...
    {
        string manifestPath = Path.Combine(CiteGmlCorpusPaths.CorpusRoot, "MANIFEST.sha256");
